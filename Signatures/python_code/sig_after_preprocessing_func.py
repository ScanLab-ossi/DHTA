...
def sig(alias): 
    ...
    sig_columns = ['doc','word','wordavg','percent_of_total','JSD','dt', 'H','I','J','K','L','M','JSD_SIGN']  
    # H local   I  global   
    # M	KL(G,M)	KL(H,M)	jsd
    ...
    for doc  in unique_files:  #loop per year 
        ...
        for index, row in filtered_df.iterrows():
            ...
            new_row = pd.DataFrame({'doc_id': [doc], 'JSD': [M],'word':[row['word']], 'dt':[doc], 'H':[H],'I':[I],'J':[J],'K':[K],'L':[L],'M':[M] ,'JSD_SIGN':[0]}) 
            print(new_row) 
            results_sig_df =  pd.concat([results_sig_df,new_row], ignore_index=True)
            # I : Global , H: local

        np.multiply(np.where(results_sig_df['I'] > results_sig_df['H'], -1, 1), results_sig_df['JSD'], out= results_sig_df['JSD_SIGN'])
    ...
    results_sig_df.to_csv(current_path+'/Signatures/results_'+alias+'/signatures_30.csv', index=True)  
